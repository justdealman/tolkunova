...
$(function() {
	...
	function setGalleryHeight() {
		$('.about-gallery__grid').each(function() {
			var t = $(this);
			var e = t.find('.about-gallery__item');
			if ( Modernizr.mq('(max-width:960px)') && Modernizr.mq('(min-width:768px)') ) {
				var max = 8;
			} else {
				var max = 6;
			}
			if ( e.size() > max ) {
				t.height(e.eq(max).offset().top-t.offset().top);
			} else {
				t.height('auto');
			}
		});
